...
$mng='<div class="alert alert-warning" role="alert"><span class="glyphicon glyphicon-warning-sign" aria-hidden="true"></span>   ' .  $lengua['REENVIO_NO']  . '</div>';
if(!$_POST){

	if($sesion->tieneSesion("id_usuario"))
	{		
        $usuario=new User_model();
		$Activado=$usuario->UsuarioActivado($_SESSION["id_usuario"]);	
...
{
	extract($_REQUEST);
	if($user->recuperarActivacion($reemail))
	{
        $mng='<div class="alert alert-success" role="alert"><span class="glyphicon glyphicon-info-sign" aria-hidden="true"></span>   ' .  $lengua['REENVIO_CORRECTO']  . '</div>';
	}
	else
	{
        $mng='<div class="alert alert-danger" role="alert"><span class="glyphicon glyphicon-warning-sign" aria-hidden="true"></span>   ' .  $lengua['REENVIO_ERROR']  . '</div>';
	}	
}
?>
                       ...
                       <span><?=$mng?></span>
						 ...
						 <form action="<?= htmlspecialchars($_SERVER["PHP_SELF"]) ?>" method="post">
                              ...
                              <div class="input-group">
                                 ...
                                 <input type="email" name="reemail" class="form-control" placeholder="Email adress" required ="" autofocus="" aria-describedby="basic-addon1">
                            </div>
                            <br />
                              <div class="row text-center">
                                ...
                                <div class="col-md-12">
                                      ...
                                      <button class="btn btn-lg btn-primary btn-block"  name="Submit" value="Login" type="Submit"><?php echo $lengua['REGISTRO_ACTIVACION']; ?></button>  
                                </div>
                            </div>
